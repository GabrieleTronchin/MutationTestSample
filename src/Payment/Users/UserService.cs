...
namespace Payment.Users
{
    public class UserService : IUserService
    {
        ...
        {
            if (user == null)
                throw new ArgumentException("User must be valid.");
        ...
        {
            if (user1 == null)
                throw new ArgumentException("Both users must be valid.");
            if (user2 == null)
                throw new ArgumentException("Both users must be valid.");
        ...
        public IReadOnlyList<IUser> GetConnectionList(IUser user1, IUser user2, uint maxLevel)
        {
            ...
            while (queue.Count > 0)
            {
                ...
                if (currentUser == user2)
                    break; // We found the destination user
            ...
            if (!distances.ContainsKey(user2))
                return new List<IUser>(); // No path found
